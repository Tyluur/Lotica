package com.runescape.utility.cache.huffman;
...
public final class Huffman {
		...
		for (int i_5_ = 0; (fileLength ^ 0xffffffff) < (i_5_ ^ 0xffffffff); i_5_++) {
			...
			if (i_6_ != 0) {
				...
				for (int i_13_ = 1 + i_6_; i_13_ <= 32; i_13_++) {
					if (i_8_ == is[i_13_]) { is[i_13_] = i_9_; }
				}
				int i_14_ = 0;
				for (int i_15_ = 0; (i_6_ ^ 0xffffffff) < (i_15_ ^ 0xffffffff); i_15_++) {
					int i_16_ = -2147483648 >>> i_15_;
					if ((i_8_ & i_16_ ^ 0xffffffff) == -1) { i_14_++; } else {
						if (huffmanAlgorithm3[i_14_] == 0) { huffmanAlgorithm3[i_14_] = i_4_; }
				...
				if ((i_4_ ^ 0xffffffff) >= (i_14_ ^ 0xffffffff)) { i_4_ = 1 + i_14_; }
			}
		}
	}
	...
	public static final int decryptMessage(byte[] messageData, int messagedDataLength, byte[] streamBuffer, int streamOffset, int messageDataOffset) {
		if ((messagedDataLength ^ 0xffffffff) == -1) { return 0; }
		int i = 0;
		messagedDataLength += messageDataOffset;
		int i_1_ = streamOffset;
		for (; ; ) {
			byte i_2_ = streamBuffer[i_1_];
			if ((i_2_ ^ 0xffffffff) <= -1) { i++; } else { i = huffmanAlgorithm3[i]; }
			int i_3_;
			if ((i_3_ = huffmanAlgorithm3[i]) < 0) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if (messagedDataLength <= messageDataOffset) { break; }
				i = 0;
			}
			if ((i_2_ & 0x40 ^ 0xffffffff) == -1) { i++; } else { i = huffmanAlgorithm3[i]; }
			if (((i_3_ = huffmanAlgorithm3[i]) ^ 0xffffffff) > -1) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if (messagedDataLength <= messageDataOffset) { break; }
				i = 0;
			}
			if ((0x20 & i_2_ ^ 0xffffffff) != -1) { i = huffmanAlgorithm3[i]; } else { i++; }
			if ((i_3_ = huffmanAlgorithm3[i]) < 0) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if (messagedDataLength <= messageDataOffset) { break; }
				i = 0;
			}
			if ((0x10 & i_2_ ^ 0xffffffff) != -1) { i = huffmanAlgorithm3[i]; } else { i++; }
			if ((i_3_ = huffmanAlgorithm3[i]) < 0) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if ((messageDataOffset ^ 0xffffffff) <= (messagedDataLength ^ 0xffffffff)) { break; }
				i = 0;
			}
			if ((i_2_ & 0x8) == 0) { i++; } else { i = huffmanAlgorithm3[i]; }
			if ((i_3_ = huffmanAlgorithm3[i]) < 0) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if (messageDataOffset >= messagedDataLength) { break; }
				i = 0;
			}
			if ((0x4 & i_2_ ^ 0xffffffff) != -1) { i = huffmanAlgorithm3[i]; } else { i++; }
			if (((i_3_ = huffmanAlgorithm3[i]) ^ 0xffffffff) > -1) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if (messageDataOffset >= messagedDataLength) { break; }
				i = 0;
			}
			if ((0x2 & i_2_) != 0) { i = huffmanAlgorithm3[i]; } else { i++; }
			if (((i_3_ = huffmanAlgorithm3[i]) ^ 0xffffffff) > -1) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if (messagedDataLength <= messageDataOffset) { break; }
				i = 0;
			}
			if ((0x1 & i_2_ ^ 0xffffffff) != -1) { i = huffmanAlgorithm3[i]; } else { i++; }
			if ((i_3_ = huffmanAlgorithm3[i]) < 0) {
				messageData[messageDataOffset++] = (byte) (i_3_ ^ 0xffffffff);
				if ((messageDataOffset ^ 0xffffffff) <= (messagedDataLength ^ 0xffffffff)) { break; }
				i = 0;
	...
	public static final int encryptMessage(int streamOffset, int messageDataLength, byte[] streamBuffer, int messageDataOffset, byte[] messageData) {
		...
		for (/**/; messageDataOffset < messageDataLength; messageDataOffset++) {
			...
			if (i_22_ == 0) { throw new RuntimeException("No codeword for data value " + i_20_); }
	...
	public static final String readEncryptedMessage(int maxLength, InputStream stream) {
		try {
			...
			if (messageDataLength > maxLength) { messageDataLength = maxLength; }
			...
			return Utils.getUnformatedMessage(messageDataLength, 0, messageData);
		} catch (Throwable e) {
			return "";
		}
	}
