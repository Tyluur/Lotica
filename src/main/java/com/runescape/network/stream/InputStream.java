package com.runescape.network.stream;
...
public final class InputStream extends Stream {
	...
	public int readBits(int bitOffset) {
		...
		if ((i_8_ ^ 0xffffffff) == (bitOffset ^ 0xffffffff)) { value += buffer[bytePos] & BIT_MASK[i_8_]; } else {
	...
	public long readQuickchatParam(int size) {
		if (--size < 0 || size > 7) {
			throw new IllegalArgumentException();
		}
		long l = 0L;
		for (int shift = 8 * size; shift >= 0; shift -= 8) {
			l |= ((long) readByte() & 0xffL) << shift;
		}
		return l;
	}
}