...
public class IComponentDefinitions {
	...
	public static IComponentDefinitions getInterfaceComponent(int id, int component) {
		...
		if (inter == null || component >= inter.length) { return null; }
		return inter[component];
	}

	public static IComponentDefinitions[] getInterface(int id) {
		if (id >= icomponentsdefs.length) { return null; }
	...
	final void decode(InputStream stream) {
		...
		if (type == 6) {
			modelType = 1;
			if ((ihash >> 16) > 1144) { anInt4864 = stream.readBigSmart(); } else {
			...
			if ((ihash >> 16) > 1144) { anInt4773 = stream.readBigSmart(); } else {
		...
		if (type == 4) {
			if ((ihash >> 16) > 1144) { anInt4759 = stream.readBigSmart(); } else {
	...
	private final int[] method4150(InputStream buffer) {
		...
		int[] is = new int[i];
		for (int i_60_ = 0; i_60_ < i; i_60_++) { is[i_60_] = buffer.readInt(); }
		return is;
	}
