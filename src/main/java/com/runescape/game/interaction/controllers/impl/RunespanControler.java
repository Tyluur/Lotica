...
import com.runescape.game.world.entity.npc.others.YellowWizard;
...
public class RunespanControler extends Controller {
	...
	private static final WorldTile[] RANDOM_LOCATIONS = { new WorldTile(3959, 6104, 1),
	...
	private static void removeRunes(Player player, boolean enter) {
		...
		for (Item item : player.getInventory().getItems().getItems()) {
			...
			if (item.getName().contains(" rune") || item.getName().contains(" essence")) {
		...
		if (didRemove) {
			player.sendMessage("Any runes you previously had on you have been banked.");
		}
	}
	...
	public void exitRunespan(boolean teleport) {
		...
		int pointsReceived = getRuneSpanPoints();

		// closes the overlay
		player.getInterfaceManager().closeOverlay();

		if (pointsReceived > 0) {
			player.getFacade().addRunespanPoints(pointsReceived);
			player.sendMessage("You receive " + Utils.format(pointsReceived) + " runespan points as a reward.");

			// players only see this message/dialogue once per session
			if (!player.getAttribute("runespawn_message", false)) {
				player.sendMessage("<col=" + ChatColors.MAROON + ">" + "Speak to the Rewards Trader at home to exchange your points for cool rewards.");
				player.putAttribute("runespawn_message", true);
			}
		}
	...
	public void sendInterfaces() {
		...
		player.getPackets().sendIComponentText(1274, 5, Integer.toString(player.getFacade().getRunespanPoints()));
		refreshInventoryPoints();
	}

	private static final double[] REAL_POINTS_ORDER = { 0.5,
	                                                    0.3,
	                                                    0.1,
	                                                    0.4,
	                                                    0.2,
	...
	public boolean processNPCClick1(NPC npc) {
		...
		switch (npc.getId()) {
			case 15402:
				...
				player.getInventory().addItem(new Item(24227, 25 - player.getInventory().getNumerOf(24227)));
				return false;
			case 15435:
			case 15434:
			case 15433:
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if (object.getId() == 70507) {
			if (object.getX() == 4367 && object.getY() == 6062) {
				...
				player.getLockManagement().lockAll(3500);
	...
	public boolean logout() {
		exitRunespan(false);
		player.setLocation(WIZARD_TOWER);
		return false;
	}
