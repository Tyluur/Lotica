...
import com.runescape.game.world.entity.npc.fightkiln.FightKilnNPC;
import com.runescape.game.world.entity.npc.fightkiln.HarAken;
import com.runescape.game.world.entity.npc.fightkiln.TokHaarKetDill;
...
public class FightKiln extends Controller {
	...
	public static void enterFightKiln(Player player, boolean quickEnter) {
		...
		if (!quickEnter) { player.getDialogueManager().startDialogue("FightKilnDialogue"); } else {
			player.getControllerManager().startController("FightKilnControler", 1);
		}
	}
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int packetId) {
		if (stage != Stages.RUNNING) { return false; }
	...
	public int[] getMap() {
		...
		if (wave < 11) { return new int[] { 504, 632 }; }
		if (wave < 21) { return new int[] { 512, 632 }; }
		if (wave < 31) { return new int[] { 520, 632 }; }
		if (wave < 34) { return new int[] { 528, 632 }; }
	...
	public void loadCave(final boolean login) {
		...
		player.getLockManagement().lockAll(); // locks player
							...
							WorldTasksManager.schedule(new WorldTask() {
								...
								public void run() {
									if (count == 0) {
										...
										player.getLockManagement().unlockAll(); // unlocks player
										stop();
									}
									count++;
								}
		...
		if (!login) { FadingScreen.fade(player, event); } else { event.run(); }
	}

	public WorldTile getMaxTile() {
		if (getCurrentWave() < 11) { return getWorldTile(49, 49); }
		if (getCurrentWave() < 21) { return getWorldTile(47, 47); }
		if (getCurrentWave() < 31) { return getWorldTile(45, 45); }
		if (getCurrentWave() < 34) { return getWorldTile(43, 43); }
		return getWorldTile(41, 41);
	}

	public WorldTile getMinTile() {
		if (getCurrentWave() < 11) { return getWorldTile(14, 14); }
		if (getCurrentWave() < 21) { return getWorldTile(16, 16); }
		if (getCurrentWave() < 31) { return getWorldTile(18, 18); }
		if (getCurrentWave() < 34) { return getWorldTile(20, 20); }
	...
	public WorldTile getTentacleTile() {
		...
		while (corner != 0 && position == 2) { position = Utils.random(5); }
		switch (corner) {
			case 0: // north
	...
	@Override
	public void moved() {
		if (stage != Stages.RUNNING || !login) { return; }
		login = false;
		setWaveEvent();
	}
	...
	public void startWave() {
		...
		int currentWave = getCurrentWave();
		player.getInterfaceManager().sendTab(player.getInterfaceManager().onResizable() ? 11 : 0, 316);
		player.getPackets().sendConfig(639, currentWave);
		if (currentWave > WAVES.length) {
			if (currentWave == 37) { aliveNPCSCount = 1; }
			return;
	...
	public void removeScene() {
		FadingScreen.fade(player, new Runnable() {
			@Override
			public void run() {
				if (stage != Stages.RUNNING) { unlockPlayer(); }
				removeTokHaarTok();
				player.getPackets().sendResetCamera();
				player.getPackets().sendBlackOut(0);
				player.getPackets().sendConfig(1241, 0);
				if (getCurrentWave() == 38) {
					Integer reward = (Integer) player.getAttributes().get("FightKilnReward");
					if (reward != null) { win(); }
				} else {
					teleportPlayerToMiddle();
					setWaveEvent();
	...
	public void removeTokHaarTok() {
		if (tokHaarHok != null) { tokHaarHok.finish(); }
	}
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		player.getDialogueManager().startDialogue("SimpleMessage", "You can't leave like this.");
		return false;
	}
	...
	public void exitCave(int type) {
		...
		if (type == 0) {
			...
			player.getPackets().closeInterface(player.getInterfaceManager().onResizable() ? 11 : 0);
			if (type == 1 || type == 4) {
	...
	public int getCurrentWave() {
		if (getArguments() == null || getArguments().length == 0) { return 0; }
		Number numbArgument = getArgument(0);
		return numbArgument.intValue();
	}

	public void setCurrentWave(int wave) {
		if (getArguments() == null || getArguments().length == 0) { this.setArguments(new Object[1]); }
		getArguments()[0] = wave;
	}
	...
	public static void useCrystal(final Player player, int id) {
		if (!(player.getControllerManager().getController() instanceof FightKiln) || player.getAttributes().get("FightKilnCrystal") != null) {
			return;
		}
			...
			public void run() {
				try {
					if (count++ == 7 || !(player.getControllerManager().getController() instanceof FightKiln)) {
						...
						player.getSkills().setLevel(Skills.DEFENCE, player.getSkills().getLevelForXp(Skills.DEFENCE));
						player.getSkills().setLevel(Skills.STRENGTH, player.getSkills().getLevelForXp(Skills.STRENGTH));
						player.getSkills().setLevel(Skills.RANGE, player.getSkills().getLevelForXp(Skills.RANGE));
						player.getSkills().setLevel(Skills.MAGIC, player.getSkills().getLevelForXp(Skills.MAGIC));
						cancel();
					} else {
						for (int i = 1; i < 7; i++) {
							if (i == skill || i == 3 || i == 5) { continue; }
							player.getSkills().setLevel(i, player.getSkills().getLevelForXp(i) / 2);
						}
						player.getSkills().setLevel(skill, (int) (player.getSkills().getLevelForXp(skill) * 1.5));
					}
				} catch (Throwable e) {
					e.printStackTrace();
				}
