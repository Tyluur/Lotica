...
import com.runescape.game.world.entity.npc.others.CastleWarBarricade;
...
public class CastleWarsPlaying extends Controller {
	...
	public void sendInterfaces() {
		player.getInterfaceManager().sendTab(player.getInterfaceManager().onResizable() ? 11 : 0, 58);
	}
			...
			public void run() {
				if (loop == 0) {
					...
					if (weaponId == 4037 || weaponId == 4039) {
						...
						if (killer != null) { killer.increaseKillCount(player); }
					}
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		if (id == 4406 || id == 4407) {
			...
			if (object.getX() == player.getX() && object.getY() == player.getY()) { return false; }
			...
			} else if (object.getY() == 9499) { player.useStairs(828, new WorldTile(2399, 3100, 0), 1, 2); }
			...
			} else if (object.getY() == 3108) { player.useStairs(827, new WorldTile(2400, 9507, 0), 1, 2); }
			player.setFreezeDelay(0);
			player.setFrozeBlocked(0);
			return false;
		}/*
	...
	public void passBarrier(WorldObject object) {
		if (object.getRotation() == 0 || object.getRotation() == 2) {
			if (player.getY() != object.getY()) { return; }
			player.getLockManagement().lockAll(2000);
			...
			if (player.getX() != object.getX()) { return; }
			player.getLockManagement().lockAll(2000);
