...
public final class QueenBlackDragonController extends Controller {
	...
	public static final WorldTile OUTSIDE = GameConstants.RESPAWN_PLAYER_LOCATION;
		...
		CoresManager.execute(() -> {
			try {
				...
				FadingScreen.unfade(player, time, () -> {
					...
					player.getInterfaceManager().sendTab(player.getInterfaceManager().onResizable() ? 1 : 0, 1285);//1133);
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		if (object.getId() == 70790) {
			...
			FadingScreen.fade(player, () -> {
				...
				boolean resizable = player.getInterfaceManager().onResizable();
		...
		if (object.getId() == 70813) {
			Magic.sendObjectTeleportSpell(player, true, GameConstants.RESPAWN_PLAYER_LOCATION);
			return false;
		}
	...
	private void end(int type) {
		 ...
		 */
		CoresManager.schedule(() -> RegionBuilder.destroyMap(regionBase[0], regionBase[1], 8, 8), 1200, TimeUnit.MILLISECONDS);
	}
