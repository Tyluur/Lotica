...
public class MakeOverMage extends Dialogue {
	...
	public void start() {
		...
		if (v == 0) {
			sendNPCDialogue(npcId, 9827, "Hello there! I am know as the Makeover Mage! I have", "spent many years researching magicks that can change", "your physical appearence.");
		} else if (v == 1) {
			stage = -2;
			sendNPCDialogue(npcId, 9827, "That is no different from what you already have. I guess I", "shouldn't charge you if I'm not changing anything.");
		} else if (v == 2) {
			stage = 19;
			sendNPCDialogue(npcId, 9827, "Whew! That was lucky.");
		}
	}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			stage = 1;
			sendOptionsDialogue(DEFAULT_OPTIONS, "Tell me more about this 'makeover'.", "Sure, do it.", "No thanks.", "Cool amulet! Can i have one?");
		} else if (stage == 1) {
			if (componentId == 1) {
				stage = 2;
			...
			stage = 10;
			sendOptionsDialogue(DEFAULT_OPTIONS, "Sure do it.", "No thanks.");
		} else if (stage == 10) {
			if (componentId == 1) {
				stage = 11;
			...
			stage = 16;
			sendOptionsDialogue(DEFAULT_OPTIONS, "Sure, here you go.", "No way! That's too expensive.");
		} else if (stage == 16) {
			if (componentId == 1) {
				if (!player.getInventory().containsItem(995, 100)) { end(); } else {
					stage = 17;
