...
import com.runescape.game.world.entity.player.InterfaceManager;
...
public abstract class Cutscene {
		...
		CoresManager.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					if (GameConstants.DEBUG) { System.out.println("Bases: " + getBaseX() + ", " + getBaseY()); }
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							CoresManager.execute(new Runnable() {
								@Override
								public void run() {
									player.getPackets().sendWindowsPane(player.getInterfaceManager().onResizable() ? InterfaceManager.RESIZABLE_WINDOW_ID : InterfaceManager.FIXED_WINDOW_ID, 0);
									if (oldData != null) {
	...
	public final boolean process(Player player) {
		...
		while (true) {
			if (constructingRegion) { return true; }
			if (stage == actions.length) {
				stopCutscene(player);
				return false;
			} else if (stage == 0) { startCutscene(player); }
	...
	public void deleteCache() {
		for (Object object : cache) { destroyCache(object); }
	}
	...
	public final void createCache(Player player) {
		...
		for (CutsceneAction action : actions) {
			if (action.getCachedObjectIndex() > lastIndex) { lastIndex = action.getCachedObjectIndex(); }
		}
