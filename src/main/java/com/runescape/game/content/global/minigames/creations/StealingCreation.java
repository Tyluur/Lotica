...
public class StealingCreation {
	...
	public static void enterTeamLobby(Player player, boolean inRedTeam) {
		if (!canEnter(player, inRedTeam)) {
			...
			CoresManager.FAST_EXECUTOR.scheduleAtFixedRate(lobbyTask = new LobbyTimer(), 0, 60000);
		player.getControllerManager().startController("StealingCreationsLobby", inRedTeam);
	}
	...
	public static void passToGame() {
		...
		for (Player player : blueTeam) {
			player.getControllerManager().startController("StealingCreationsGame", false);
		}
	}
