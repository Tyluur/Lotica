...
public class Crucible {
	...
	public static boolean isImmune(Player player) {
		Long immune = (Long) player.getAttributes().get("CrucibleImmune");
	...
	public static void setImmune(Player player, int seconds) {
		if(seconds == 0)
			...
			player.getAttributes().put("CrucibleImmune", (Utils.currentTimeMillis() +  seconds*1000));
	}
	...
	public static void payBountyFee(Player player, CrucibleControler controler) {
		Fissures fissure = (Fissures) player.getAttributes().remove("crucibleBounty");
		if(fissure == null)
			return;
