...
public class DuelRules {
	...
	public boolean canAccept(ItemsContainer<Item> stake) {
		if (getRule(0) && getRule(1) && getRule(2)) {
			player.getPackets().sendGameMessage("You have to be able to use at least one combat style in a duel.", true);
			return false;
		}
		int count = 0;
		Item item;
	...
	public void setRules(int ruleId, boolean updated) {
		...
		if (updated) {
			...
			player.getControllerManager().verifyControlerForOperation(DuelArena.class).ifPresent(DuelArena::cancelAccepted);
			//target.getControllerManager().verifyControlerForOperation(DuelArena.class).ifPresent(DuelArena::cancelAccepted);
		}
		setConfigs();
	}
	...
	public boolean isReceivedWins() {
		return receivedWins;
	}

	public void setReceivedWins(boolean receivedWins) {
		this.receivedWins = receivedWins;
	}

	public boolean[] getDuelRuels() {
		return duelRules;
	}
}
