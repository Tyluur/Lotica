 ...
 * @since May 18, 2015
 */
public class MacMute extends CommandSkeleton<String> {
		...
		player.getPackets().requestClientInput(new InputEvent("Enter amount of hours", InputEventType.INTEGER) {
			...
			public void handleInput() {
				GsonStartup.getOptional(PunishmentLoader.class).ifPresent(c -> {
					c.addPunishment(targetPlayer.getMacAddress(), PunishmentType.MACMUTE, TimeUnit.HOURS.toMillis(((Integer) getInput()).longValue()));
					player.sendMessage("Successfully ipmuted " + targetPlayer.getUsername() + "!", false);
				});
			}
		});
	}
