 ...
 * @since May 18, 2015
 */
public class Mute extends CommandSkeleton<String> {
		...
		player.getPackets().requestClientInput(new InputEvent("Enter amount of hours", InputEventType.INTEGER) {
			...
			public void handleInput() {
				GsonStartup.getOptional(PunishmentLoader.class).ifPresent(c -> {
					c.addPunishment(targetPlayer.getUsername(), PunishmentType.MUTE, TimeUnit.HOURS.toMillis(((Integer) getInput()).longValue()));
					player.sendMessage("Successfully muted " + targetPlayer.getUsername() + "!", false);
				});
			}
		});
	}
