...
public final class RequestController extends Controller {
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int packetId) {
		ClanWars clanWars = (ClanWars) player.getAttributes().get("clan_wars");
		if (clanWars == null) {
			return true;
		}
	...
	public void decline() {
		Player other = (Player) player.getAttributes().get("clan_request_p");
		ClanWars war = (ClanWars) player.getAttributes().get("clan_wars");
	...
	private static boolean canRequest(Player player, Player target, boolean message) {
		...
		} else if (player.getCurrentFriendChat().getRank(player.getPrimaryRight(), player.getUsername()) < 3) {
			text = "You need to be a higher rank to start a war.";
		} else if (target.getCurrentFriendChat().getRank(target.getPrimaryRight(), target.getUsername()) < 3) {
