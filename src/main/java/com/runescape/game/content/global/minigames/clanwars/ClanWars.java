...
public final class ClanWars implements Serializable {
	...
	public void switchRule(Rules rule, Player player) {
		Player other = (Player) player.getAttributes().get("clan_request_p");
		if (other == null || player.getAttributes().get("clan_wars") != other.getAttributes().get("clan_wars")) {
			return;
		}
	...
	public void accept(final Player player) {
		final Player other = (Player) player.getAttributes().get("clan_request_p");
		if (other != null && other.getAttributes().get("accepted_war_terms") == Boolean.TRUE) {
			CoresManager.submit((Runnable) () -> {
				try {
					player.getAttributes().remove("accepted_war_terms");
					other.getAttributes().remove("accepted_war_terms");
					...
					CoresManager.FAST_EXECUTOR.scheduleAtFixedRate(timer = new ClanWarsTimer(ClanWars.this), 600, 600);
					enter(player);
					enter(other);
	...
	public static void enter(Player p) {
		...
		final ClanWars c = hasWar ? p.getCurrentFriendChat().getClanWars() : (ClanWars) p.getAttributes().get("view_clan");
		if (c == null) {
			return;
		}
		...
		if (hasWar && c.timer.isStarted() && c.isKnockOut()) {
			...
			p.getAttributes().put("view_prefix", c.firstTeam == p.getCurrentFriendChat() ? 0 : 1);
		}
		if (hasWar) {
		...
		Integer prefix = (Integer) p.getAttributes().get("view_prefix");
	...
	public void leave(Player p, boolean ingame) {
		...
		boolean resized = p.getInterfaceManager().onResizable();
