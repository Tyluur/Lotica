...
public final class WarControler extends Controller {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					if (clanWars.getFirstTeam().equals(player.getCurrentFriendChat())) {
	...
	@Override
	public void process() {
		if (!player.getInterfaceManager().containsInterface(265)) { player.getInterfaceManager().sendOverlay(265); }
	}
