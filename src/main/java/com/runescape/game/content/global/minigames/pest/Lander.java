...
import com.runescape.game.content.global.minigames.pest.PestControl.PestData;
...
public class Lander {
	...
	private static final int AUTO_GAME = GameConstants.DEBUG ? 1 : 15;

	private static final int TIME = 150;

	private List<Player> lobby = Collections.synchronizedList(new LinkedList<>());
	...
	public class LobbyTimer extends TimerTask {
		...
		public void run() {
			if (seconds == 0 && lobby.size() >= 5 || lobby.size() >= AUTO_GAME) {
				passPlayersToGame();
			} else if (seconds == 0) { seconds = TIME; } else if (lobby.size() == 0) {
				cancel();
				return;
			}
			seconds--;
			if (seconds % 30 == 0) { refreshLanderInterface(); }
		}
	...
	public void enterLander(Player player) {
		if (lobby.size() == 0) { CoresManager.FAST_EXECUTOR.schedule(timer = new LobbyTimer(), 1000, 1000); }
	...
	private void refreshLanderInterface() {
		for (Player teamPlayer : lobby) { teamPlayer.getControllerManager().getController().sendInterfaces(); }
	}
	...
	public enum LanderRequirement {
		...
		static {
			for (LanderRequirement lander : LanderRequirement.values()) { landers.put(lander.getId(), lander); }
		}
	...
	static {
		for (int i = 0; i < landers.length; i++) { landers[i] = new Lander(LanderRequirement.forId(i)); }
	}
