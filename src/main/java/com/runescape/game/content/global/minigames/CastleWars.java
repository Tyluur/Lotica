...
import com.runescape.game.interaction.dialogues.impl.minigame.CastleWarsScoreboard;
...
public final class CastleWars {
	...
	public static void createPlayingGame() {
		...
		CoresManager.FAST_EXECUTOR.scheduleAtFixedRate(playingGame, 60000, 60000);
	...
	public static void refreshTimeLeft(Player player) {
		player.getPackets().sendConfig(380, playingGame == null ? 0 : playingGame.minutesLeft - (player.getControllerManager().getController() instanceof CastleWarsPlaying ? 5 : 0));
	}
	...
	public static void joinPlayingGame(Player player, int team) {
		...
		player.getPackets().closeInterface(player.getInterfaceManager().onResizable() ? 34 : 0);
