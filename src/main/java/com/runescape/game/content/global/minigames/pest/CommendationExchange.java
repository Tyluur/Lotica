...
public class CommendationExchange {//1875 TODO
	...
	public static final int INTERFACE = 1011, RATE_ONE = 1, RATE_TEN = 10, RATE_HUNDRED = 100;
	...
	private static final int[] SKILLS = { Skills.STRENGTH, Skills.DEFENCE, Skills.HITPOINTS, Skills.RANGE, Skills.MAGIC, Skills.PRAYER };
	...
	public static void handleButtonOptions(Player player, int componentId) {
		if (componentId == 68) {
			...
			if (player.getSkills().getLevelForXp(Skills.HERBLORE) < 25) {
				...
				return;
			} else if (!exchangeCommendation(player, 30)) { return; }
			...
			if (player.getSkills().getLevelForXp(Skills.MINING) < 25) {
				...
				return;
			} else if (!exchangeCommendation(player, 15)) { return; }
			...
			for (int index = 0; index < VOID_BASE_COMPONENTS.length; index++) {
				if (VOID_BASE_COMPONENTS[index] == componentId) { addVoidItem(player, index); }
			}

			for (int index = 0; index < CHARM_BASE_COMPONENTS.length; index++) {
				int charmComponent = CHARM_BASE_COMPONENTS[index];
				for (int i = 0; i < 6; i += 2) {
					if (charmComponent + i == componentId) { addCharm(player, CHARMS[index], getRateForIndex(i / 2)); }
				}
			}
		}
	}
	...
	private static void addCharm(Player player, int itemId, int rate) {
		if (rate == 100) { rate = player.getInventory().getFreeSlots(); }
	...
	private static int getRateForIndex(int index) {
		if (index == 0) { return RATE_ONE; } else if (index == 1) { return RATE_TEN; } else if (index == 2) {
			return RATE_HUNDRED;
		}
		return 0;
	}
	...
	private static void addXPForSkill(Player player, int skill, int rate) {
		...
		player.getSkills().addXpNoModifier(skill, experience / 18);
		player.getDialogueManager().startDialogue("SimpleMessage", "You gain " + Utils.format((int) experience) + " experience in " + Skills.SKILL_NAME[skill] + ".");
	}

	private static void addVoidItem(Player player, int index) {
		if (!player.getSkills().hasRequirements(Skills.ATTACK, 42, Skills.STRENGTH, 42, Skills.DEFENCE, 42, Skills.HITPOINTS, 42, Skills.RANGE, 42, Skills.MAGIC, 42, Skills.PRAYER, 22)) {
		...
		if (!exchangeCommendation(player, cost)) { return; }
	...
	private static double calculateExperience(Player player, int skill) {
		...
		int constant = 35;
		if (skill == Skills.MAGIC || skill == Skills.RANGE) { constant = 32; } else if (skill == Skills.PRAYER) {
			constant = 18;
		}
