...
public class LeatherCrafting extends Action {
	...
	public static final int PRODUCTS[][] = { { 1065, 1099, 1135 }, { 2487, 2493, 2499 }, { 2489, 2495, 2501 }, { 2491, 2497, 2503 }, { 24376, 24379, 24381 } };
	...
	public enum LeatherData {
		...
		LeatherData(int leatherId, int leatherAmount, int finalProduct, int requiredLevel, double experience) {
			...
			this.name = ItemDefinitions.forId(getFinalProduct()).getName().replace("d'hide", "");
		}
	...
	public static boolean handleItemOnItem(Player player, Item itemUsed, Item usedWith) {
		for (int aLEATHER : LEATHER) {
			if (itemUsed.getId() == aLEATHER || usedWith.getId() == aLEATHER) {
				player.getAttributes().put("leatherType", aLEATHER);
	...
	public static int getIndex(Player player) {
		int leather = (Integer) player.getAttributes().get("leatherType");
	...
	private boolean checkAll(Player player) {
		...
		if (!player.getInventory().containsOneItem(data.getLeatherId())) {
			player.getPackets().sendGameMessage("You've ran out of " + ItemDefinitions.forId(data.getLeatherId()).getName().toLowerCase() + ".");
			return false;
		}
		return true;
	}
