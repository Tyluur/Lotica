...
import com.runescape.game.world.entity.npc.others.DreadNip;
...
public class Summoning {
	...
	public enum Pouches {
		...
		/**
		 * This method finds as pouch by the npc id
		 *
		 * @param npcId
		 * 		The id of the npc
		 */
		public static Pouches findByNpcId(int npcId) {
			for (Pouches pouch : Pouches.values()) {
				if (pouch.getNpcId() == npcId) {
					return pouch;
				}
			}
			return null;
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		try {
			return (Familiar) Class.forName("com.runescape.game.world.entity.npc.familiar.impl." + (NPCDefinitions.getNPCDefinitions(pouch.getNpcId())).getName().replace(" ", "").replace("-", "")).getConstructor(Player.class, Pouches.class, WorldTile.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
		} catch (Throwable e) {
			e.printStackTrace();
			return null;
		}
	...
	public static void spawnFamiliar(Player player, Pouches pouch) {
		...
		ItemDefinitions def = ItemDefinitions.forId(pouch.getPouchId());
		if (def == null) {
			player.sendMessage("Error with this pouch, please report this on forums.");
			return;
		}
		...
		if (skillReq != null) {
			for (int skillId : skillReq.keySet()) {
				...
				if (player.getSkills().getLevelForXp(skillId) < level) {
					if (hasRequiriments) {
						player.sendMessage("You are not high enough level to use this pouch.");
					}
					hasRequiriments = false;
					String name = Skills.SKILL_NAME[skillId].toLowerCase();
					player.sendMessage("You need to have a" + (name.startsWith("a") ? "n" : "") + " " + name + " level of " + level + " to summon this.");
					sentMessage = true;
				}
			}
		}
		if (!hasRequiriments || player.getSkills().getLevelForXp(Skills.SUMMONING) < pouch.getLevel()) {
			String name = Skills.SKILL_NAME[Skills.SUMMONING].toLowerCase();
			if (!sentMessage) {
				player.sendMessage("You need to have a" + (name.startsWith("a") ? "n" : "") + " " + name + " level of " + pouch.getLevel() + " to summon this.");
			}
			return;
		}
			...
			if (itemReq != null) {
				for (int reqId : itemReq.keySet()) {
					...
					if (!player.getInventory().containsItem(reqId, amount)) {
						hasRequiriments = false;
						String name = ItemDefinitions.forId(reqId).getName();
	...
	public static void openDreadnipInterface(Player player) {
		...
		player.getInterfaceManager().sendTab(player.getInterfaceManager().onResizable() ? 115 : 175, 1165);
	}
