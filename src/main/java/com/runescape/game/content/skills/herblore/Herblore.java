package com.runescape.game.content.skills.herblore;

import com.runescape.game.world.entity.masks.Animation;
import com.runescape.game.world.entity.player.Player;
import com.runescape.game.world.entity.player.Skills;
import com.runescape.game.world.entity.player.achievements.AchievementHandler;
import com.runescape.game.world.entity.player.achievements.elite.System_Overload;
...
public class Herblore extends Action {
	...
	public boolean process(Player player) {
		...
		if (!player.getInventory().containsItem(node.getId(), ingredients == Ingredients.GRENWALL_SPIKES && node.getId() == ingredients.getItemId() ? 5 : 1) || !player.getInventory().containsItem(otherItem.getId(), ingredients == Ingredients.GRENWALL_SPIKES && otherItem.getId() == ingredients.getItemId() ? 5 : 1)) {
			//System.out.println(player.getInventory().containsItem(node.getId(), node.getAmount()) + ", " + player.getInventory().containsItem(otherItem.getId(), otherItem.getAmount()) + "\t[node=" + node + ", other=" + otherItem + "])");
			return false;
		}
		//System.out.println("we all good bro");
		return true;
	}
	...
	public int processWithDelay(Player player) {
		...
		if (otherItem.getId() == VIAL || otherItem.getId() == COCONUT_MILK || node.getId() == VIAL || node.getId() == COCONUT_MILK) {
			...
			player.getPackets().sendGameMessage("You crush the " + (node.getId() == PESTLE_AND_MORTAR ? otherItem.getDefinitions().getName().toLowerCase() : node.getDefinitions().getName().toLowerCase()) + " with your pestle and mortar.", true);
		} else if (ingredients == Ingredients.TORSTOL && otherItem.getId() != VIAL) {
			int[] overloadItems = new int[] { 15309, 15313, 15317, 15321, 15325, 269 };
			if (player.getInventory().removeItems(1, overloadItems)) {
				player.getPackets().sendGameMessage("You combine the torstol with the potions and get an overload.");
				AchievementHandler.incrementProgress(player, System_Overload.class);
				removedItems = true;
			}
		} else {
		...
		if (!removedItems) {
			if (node.getId() == PESTLE_AND_MORTAR) {
				...
				if (ingredients == Ingredients.GRENWALL_SPIKES) {
					player.getInventory().removeItems(new Item(node.getId(), node.getId() == ingredients.getItemId() ? 5 : 1), rawIngredient == null ? new Item(otherItem.getId(), otherItem.getId() == ingredients.getItemId() ? 5 : 1) : null);
				} else {
