...
import com.runescape.game.world.entity.npc.dungeonnering.Guardian;
...
public class DungeonManager {
	...
	public WorldTile getRoomCenterTile(RoomReference reference) {
		return new WorldTile(((boundChuncks[0] << 3) + reference.getX() * 16) + 8, ((boundChuncks[1] << 3) + reference.getY() * 16) + 8, 0);
	}
	...
	public void linkPartyToDungeon() {
		...
		for (DungeonPartyPlayer dplayer : party.getTeam()) {
			Player player = dplayer.getPlayer();
			player.getControllerManager().startController("DungeonControler", this);
			player.stopAll();
			player.reset();
	...
	public void spawnNPC(RoomReference reference, int id, int x, int y, boolean check, boolean guardian) {
		...
		WorldTile tile = new WorldTile(((boundChuncks[0] << 3) + reference.getX() * 16) + x, ((boundChuncks[1] << 3) + reference.getY() * 16) + y, 0);
