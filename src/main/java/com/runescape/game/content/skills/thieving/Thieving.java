...
public class Thieving {
	...
	public static boolean pickDoor(Player player, WorldObject object) {
		if (player.getAttributes().get("numbFingers") == null) { player.getAttributes().put("numbFingers", 0); }
		...
		if (object.matches(3190, 3957, 0)) {
			boolean outside = player.matches(3190, 3957, 0);
			player.setNextWorldTile(new WorldTile(3190, outside ? 3958 : 3957, 0));
		} else if (object.matches(3191, 3963, 0)) {
			boolean outside = player.matches(3191, 3963, 0);
			player.setNextWorldTile(new WorldTile(3191, outside ? 3962 : 3963, 0));
		} else {
