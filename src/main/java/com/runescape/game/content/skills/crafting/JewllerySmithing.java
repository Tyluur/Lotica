...
public class JewllerySmithing {
	...
	private static final int[][] ITEMS = { { 1635, 1637, 1639, 1641, 1643, 1645, 1647 }, { 1654, 1656, 1658, 1660, 1662, 1664, 1666 }, { 1673, 1675, 1677, 1679, 1681, 1685, 6579 }, { 11069, 11072, 11076, 11085, 11092, 11115, 11067 } };
	...
	public static void openInterface(Player player) {
		...
		for (int primaryIndex = 0; primaryIndex < MOLDS.length; primaryIndex++) {
			if (player.getInventory().contains(MOLDS[primaryIndex])) {
				...
				for (int secondaryIndex = 0; secondaryIndex < ITEMS[primaryIndex].length; secondaryIndex++) {
					player.getPackets().sendItems((299 + (primaryIndex * 14) + secondaryIndex), new Item[] { new Item(!player.getInventory().contains(GEMS[secondaryIndex]) ? NULL_ITEMS[primaryIndex] : ITEMS[primaryIndex][secondaryIndex]) });
				}
			} else
	...
	public static void ringTransformation(Player player, final int itemId) {
		...
		player.getActionManager().setAction(new Action() {
			...
			@Override
			public boolean handleClick(Player player, int interfaceId, int componentId) {
				if (interfaceId == 375 && componentId == 3) {
					stop(player);
					return true;
				}
				return false;
