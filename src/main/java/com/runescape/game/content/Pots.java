...
import com.runescape.game.interaction.controllers.impl.CrucibleControler;
...
public final class Pots {
	...
	public static boolean pot(Player player, Item item, int slot) {
		...
		if (player.getLockManagement().isLocked(LockType.POTIONS)) {
			return true;
		}
		if (!player.getControllerManager().canPot(pot)) { return true; }
		if (!pot.effect.canDrink(player)) { return true; }
		player.getLockManagement().lockActions(1075, LockType.POTIONS);
		...
		for (int skillId : pot.effect.affectedSkills) {
			player.getSkills().setLevel(skillId, pot.effect.getAffectedSkill(player, skillId, player.getSkills().getLevel(skillId), player.getSkills().getLevelForXp(skillId)));
		}
	...
	public static boolean mixPotion(Player player, int usedSlot, int withSlot) {
		...
		Pot first = getPot(itemUsed.getApplicableIds());
		Pot second = getPot(usedWith.getApplicableIds());
		...
		if (first.effect != second.effect)
			...
			return false;
		} else if (itemUsed.getApplicableIds() == 229)// empty vial
			return false;*/
		return true;
	}

	public static void resetOverLoadEffect(Player player) {
		if (Wilderness.isAtWild(player) || player.getControllerManager().verifyControlerForOperation(DuelArena.class).isPresent() || player.getControllerManager().verifyControlerForOperation(Wilderness.class).isPresent()) {
	...
	public static void applyOverLoadEffect(Player player) {
		if (Wilderness.isAtWild(player) || player.getControllerManager().verifyControlerForOperation(DuelArena.class).isPresent() || player.getControllerManager().verifyControlerForOperation(Wilderness.class).isPresent()) {
			resetOverLoadEffect(player);
			return;
		}
		if (player.getControllerManager().getController() instanceof CrucibleControler || FfaZone.isOverloadChanged(player)) {
			int actualLevel = player.getSkills().getLevel(Skills.ATTACK);
			int realLevel = player.getSkills().getLevelForXp(Skills.ATTACK);
			int level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.ATTACK, (int) (level + 5 + (realLevel * 0.15)));

			actualLevel = player.getSkills().getLevel(Skills.STRENGTH);
			realLevel = player.getSkills().getLevelForXp(Skills.STRENGTH);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.STRENGTH, (int) (level + 5 + (realLevel * 0.15)));

			actualLevel = player.getSkills().getLevel(Skills.DEFENCE);
			realLevel = player.getSkills().getLevelForXp(Skills.DEFENCE);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.DEFENCE, (int) (level + 5 + (realLevel * 0.15)));

			actualLevel = player.getSkills().getLevel(Skills.MAGIC);
			realLevel = player.getSkills().getLevelForXp(Skills.MAGIC);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.MAGIC, level + 5);

			actualLevel = player.getSkills().getLevel(Skills.RANGE);
			realLevel = player.getSkills().getLevelForXp(Skills.RANGE);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.RANGE, (int) (level + 5 + (realLevel * 0.1)));
		} else {
			int actualLevel = player.getSkills().getLevel(Skills.ATTACK);
			int realLevel = player.getSkills().getLevelForXp(Skills.ATTACK);
			int level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.ATTACK, (int) (level + 5 + (realLevel * 0.22)));

			actualLevel = player.getSkills().getLevel(Skills.STRENGTH);
			realLevel = player.getSkills().getLevelForXp(Skills.STRENGTH);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.STRENGTH, (int) (level + 5 + (realLevel * 0.22)));

			actualLevel = player.getSkills().getLevel(Skills.DEFENCE);
			realLevel = player.getSkills().getLevelForXp(Skills.DEFENCE);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.DEFENCE, (int) (level + 5 + (realLevel * 0.22)));

			actualLevel = player.getSkills().getLevel(Skills.MAGIC);
			realLevel = player.getSkills().getLevelForXp(Skills.MAGIC);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.MAGIC, level + 7);

			actualLevel = player.getSkills().getLevel(Skills.RANGE);
			realLevel = player.getSkills().getLevelForXp(Skills.RANGE);
			level = actualLevel > realLevel ? realLevel : actualLevel;
			player.getSkills().setLevel(Skills.RANGE, (int) (level + 4 + (Math.floor(realLevel / 5.2))));
		}
	}
	...
	private enum Effects {
		...
		ZAMORAK_BREW(Skills.ATTACK) {
			...
			public void extra(Player player) {
				...
				player.getSkills().setLevel(Skills.ATTACK, toAdd);
			}
		...
		SUMMONING_POT(Skills.SUMMONING) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				...
				if (actualLevel + restore > realLevel) { return realLevel; }
			...
			public void extra(Player player) {
				...
				if (familiar != null) { familiar.restoreSpecialAttack(15); }
			}
		},
		ANTIPOISON() {
			@Override
		...
		ENERGY_POTION() {
			@Override
			public void extra(Player player) {
				int restoredEnergy = player.getEnergyValue() + 20;
		...
		SUPER_ENERGY() {
			@Override
			public void extra(Player player) {
				int restoredEnergy = player.getEnergyValue() + 40;
				player.setRunEnergy(restoredEnergy > 100 ? 100 : restoredEnergy);
			}
		},
		ANTI_FIRE() {
