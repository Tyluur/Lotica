...
import com.runescape.game.world.entity.player.achievements.medium.Munchies;
...
public class Foods {
	...
	public static boolean eat(final Player player, Item item, int slot) {
		...
		if (player.getLockManagement().isLocked(LockType.FOOD)) {
			if (food.equals(Food.KARAMBWANI)) {
				if (player.getAttribute("karambwani_eat") != null) {
					if (System.currentTimeMillis() - (long) player.getAttribute("karambwani_eat") <= 800) {
						return true;
					}
				}
				player.putAttribute("karambwani_eat", System.currentTimeMillis());
			} else { return true; }
		}
		String name = ItemDefinitions.forId(food.getId()).getName().toLowerCase();
		...
		player.getLockManagement().lockActions(foodDelay, LockType.FOOD);
		...
		AchievementHandler.incrementProgress(player, Munchies.class, 1);
		return true;
	}
	...
	public enum Food {
		...
		PURPLE_SWEETS(10476, Utils.percentageChance(30) ? 30 : Utils.percentageChance(50) ? 20 : 10, Effect.PURPLE_SWEET_EFFECT),
	...
	public enum Effect {
		SUMMER_PIE {
			...
			public void effect(Object object) {
				...
				player.getSkills().setLevel(Skills.AGILITY, level >= realLevel ? realLevel + 5 : level + 5);
			}
		...
		GARDEN_PIE {
			...
			public void effect(Object object) {
				...
				player.getSkills().setLevel(Skills.FARMING, level >= realLevel ? realLevel + 3 : level + 3);
			}
		...
		FISH_PIE {
			...
			public void effect(Object object) {
				...
				player.getSkills().setLevel(Skills.FISHING, level >= realLevel ? realLevel + 3 : level + 3);
			}
		},
		...
		ADMIRAL_PIE {
			...
			public void effect(Object object) {
				...
				int level = player.getSkills().getLevel(Skills.FISHING);
				int realLevel = player.getSkills().getLevelForXp(Skills.FISHING);
				player.getSkills().setLevel(Skills.FISHING, level >= realLevel ? realLevel + 5 : level + 5);
			}
		},
		...
		WILD_PIE {
			...
			public void effect(Object object) {
				...
				player.getSkills().setLevel(Skills.SLAYER, level >= realLevel ? realLevel + 4 : level + 4);
				int level2 = player.getSkills().getLevel(Skills.RANGE);
				int realLevel2 = player.getSkills().getLevelForXp(Skills.RANGE);
				player.getSkills().setLevel(Skills.RANGE, level2 >= realLevel2 ? realLevel2 + 4 : level2 + 4);
			}
		},
		...
		SPICY_STEW_EFFECT {
			...
			public void effect(Object object) {
				...
				if (Utils.random(100) > 5) {
					...
					player.getSkills().setLevel(Skills.COOKING, level >= realLevel ? realLevel + 6 : level + 6);
				} else {
					int level = player.getSkills().getLevel(Skills.COOKING);
					player.getSkills().setLevel(Skills.COOKING, level <= 6 ? 0 : level - 6);
				}
			}
		...
		PURPLE_SWEET_EFFECT {
			...
			public void effect(Object object) {
				Player player = (Player) object;
				player.setRunEnergy(player.getRunEnergy() + (player.getRunEnergy() * 0.10));
			}
		};
