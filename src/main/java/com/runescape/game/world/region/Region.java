...
import com.runescape.utility.world.object.ObjectRemoval.CustomObject;
...
public class Region {
	...
	protected Map<String, Integer> timeInRegion;
	...
	private void loadRemoved() {
		for (int i = 0; i <= 10; i++) {
			for (int j = 0; j <= 10; j++) {
				for (CustomObject obj : ObjectRemoval.getRemovedAtRegion(regionId)) {
					removedObjects.add(new WorldObject(obj.getId(), i, j, obj.getTile()));
				}
			}
		}
	}
	...
	private void loadNPCSpawns() {
		NPCSpawnLoader.loadSpawns(regionId);
	}
	...
	public void clip(WorldObject object, int x, int y, boolean... args) {
		if (map == null) {
			map = new RegionMap(regionId, false);
		}
		if (clipedOnlyMap == null) {
	...
	public void spawnObject(WorldObject object, int plane, int localX, int localY, boolean original) {
		...
		CustomObject customRemovedObject = ObjectRemoval.getCustomObject(object, false);
		int slot = OBJECT_SLOTS[object.getType()];
		if (original) {
			if (customRemovedObject != null) {
//				System.out.println("Didn't spawn " + object + " bc removal and set mask to 0");
				unclip(object.getPlane(), object.getX(), object.getY());
				removedObjects.add(object);
				return;
			}
			objects[plane][localX][localY][slot] = object;
			clip(object, localX, localY, true);
		} else {
			...
			// if there is a removed object on this tile
			if (removed != null) {
				...
				if (customRemovedObject == null) {
					...
					object.setSpawned(false);
					//System.out.println("Spawning removed object:\t" + object);
				}
				// if an object wasn't removed on this tile, we can spawn it
				if (customRemovedObject != null) {
					spawnedObjects.add(object);
					object.setSpawned(true);
//					System.out.println("Adding spawned object:\t" + object);
				}
	...
	public void removeObject(WorldObject object, int plane, int localX, int localY) {
		...
		CustomObject customObject = ObjectRemoval.getCustomObject(object, true);
		...
		if (spawned != null) {
		...
		} else if (objects != null && objects[plane] != null && objects[plane][localX] != null && objects[plane][localX][localY] != null && objects[plane][localX][localY][slot] != null && objects[plane][localX][localY][slot].equals(object)) { // removes
			// original
			unclip(object, localX, localY);
			removedObjects.add(object);
			...
			removedObjects.add(object);
			/*
			 * object = spawned; spawnedObjects.remove(object); unclip(object,
			 * localX, localY); if (objects[plane][localX][localY][slot] !=
			 * null) {// original // unclips non original to clip original above
			 * clip(objects[plane][localX][localY][slot], localX, localY);
			 * original = objects[plane][localX][localY][slot]; }
			 */
		} else {
	...
	public void addPlayerIndex(Player player) {
		...
		timeInRegion.put(player.getUsername(), 0);
	}
	...
	public void removePlayerIndex(Player player) {
		if (playersIndexes == null) {
			return;
		}
		Integer index = player.getIndex();
		playersIndexes.remove(index);
		timeInRegion.remove(player.getUsername());
	}
	...
	public void increaseTimeSpent(Player player) {
		Integer time = timeInRegion.get(player.getUsername());
		timeInRegion.put(player.getUsername(), (time == null ? 1 : time + 1));
	}
