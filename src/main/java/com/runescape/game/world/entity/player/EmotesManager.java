...
public final class EmotesManager implements Serializable {
	...
	public EmotesManager() {
		...
		for (int emoteId = 2; emoteId < 24; emoteId++) { unlockedEmotes.add(emoteId); }
	...
	public static int getId(int slotId, int packetId) {
		...
		switch (slotId) {
			...
			case 1:
				return 3;
			case 2:
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) { return 4; } else {
	...
	public void useBookEmote(int slotId, int id) {
		...
		Emotes emote = Emotes.getEmoteBySlot(id);
		if (emote != null) {
			TreasureTrailHandler.handleEmote(player, emote);
		}
//		System.out.println("emote=" + emote + "\tslotId=" + slotId + ", id=" + id);
		//unlockEmote(id);
		player.stopAll(false);
		if (!unlockedEmotes.contains(id)) {
			unlockEmote(id);
			useBookEmote(slotId, id);
	...
	public enum Emotes {
		 ...
		 * 		The slot id
		 */
		public static Emotes getEmoteBySlot(int slotId) {
