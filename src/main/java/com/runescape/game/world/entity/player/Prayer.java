...
import com.runescape.game.content.global.minigames.clanwars.ClanWars.Rules;
...
public class Prayer implements Serializable {
	...
	private final static double[][] prayerDrainRate = {
			{ 0.6, 0.6, 0.6, 0.6, 0.9, 0.6, 0.6, 6, 6, 6, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.6, 0.3, 1, 1, 0.6, 0.6, 0.8, 0.6, 0.4, 0.8, 0.8, 0.8 },
			{ 0.6, 0.6, 0.6, 0.6, 0.6,
					0.6, 1.2, 1.2, 1.2, 1.2,
					0.4, 0.4, 0.4, 0.4, 0.3,
					0.3, 0.3, 1.2, 1.6, 1.3 } };
	...
	private transient double[][] drainData;

	private transient long[][] drainTimes;
	...
	public void switchQuickPrayers() {
		...
		if (!hasQuickPrayers()) {
			player.sendMessage("You have no quick-prayers to activate.", true);
		} else {
	...
	private boolean checkPrayer() {
		if (prayerPoints <= 0) {
			...
			player.getPackets().sendGameMessage("You have ran out of prayer points.");
			return false;
		}
		return true;
	}
	...
	private boolean hasQuickPrayers() {
		for (boolean prayer : quickPrayers[getPrayerBook()]) {
			if (prayer) {
				return true;
			}
	...
	private boolean usePrayer(int prayerId) {
		...
		if (getPrayerBook() == 0) {
			...
			if (prayerId == 28 || prayerId == 29) {
				boolean unlocked = player.getFacade().getUnlockedPrayers()[prayerId == 28 ? 0 : 1];
				if (!unlocked) {
					player.sendMessage("You must unlock this prayer by reading a scroll of " + (prayerId == 28 ? "rigour" : "augury") + ".");
					return false;
				}
			}
...
//			resetDrainPrayer(prayerId);
	...
	public void processPrayerDrain() {
		...
		for (int slot = 0; slot < onPrayers[book].length; slot++) {
			if (!onPrayers[book][slot]) {
				continue;
			}
//			long lastDrainTime = drainTimes[book][slot];
//			System.out.println("lastDrainTime=" + lastDrainTime + ", " + (System.currentTimeMillis() - lastDrainTime) + ", slot=" + slot);
//			if (lastDrainTime <= 0 || (System.currentTimeMillis() - lastDrainTime) >= TimeUnit.SECONDS.toMillis(1)) {
			drainData[book][slot] += prayerDrainRate[book][slot] * 2;

//				System.out.println("amountToDrain=" + drainData[book][slot]);

			if (drainData[book][slot] >= 1) {
				int amount = (int) Math.ceil(drainData[book][slot]);
//				System.out.println("amount=" + amount + " slot=" + slot + " count=" + prayerDrainRate[book][slot]);

//					System.out.println("amount=" + amount + ", slot=" + slot);

				// sets the last time we drained prayer
//					drainTimes[book][slot] = System.currentTimeMillis();

				// reduces the amount to drain
				drainData[book][slot] = drainData[book][slot] - amount;
				drainPrayer(amount);
			}
//			}
		}
		if (!checkPrayer()) { closeAllPrayers(); }
	}
	...
	public void setPlayer(Player player) {
		...
		leechBonuses = new int[11];
		drainTimes = new long[2][30];
		drainData = new double[2][30];
	}
