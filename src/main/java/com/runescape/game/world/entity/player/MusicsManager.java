...
public final class MusicsManager implements Serializable {
	...
	public void passMusics(Player p) {
		for (int musicId : p.getMusicsManager().unlockedMusics) {
			if (!unlockedMusics.contains(musicId)) { unlockedMusics.add(musicId); }
		}
	}
	...
	public void addToPlayList(int musicIndex) {
		...
		if (musicId != -1 && unlockedMusics.contains(musicId) && !playList.contains(musicId)) {
			...
			if (playListOn) { switchPlayListOn(); } else { refreshPlayListConfigs(); }
		}
	}
	...
	public void init() {
		...
		if (playingMusic >= 0) { playMusic(playingMusic); }
		refreshListConfigs();
		refreshPlayListConfigs();
	}
	...
	public void replayMusic() {
		if (playListOn && playList.size() > 0) {
			if (shuffleOn) { playingMusic = playList.get(Utils.getRandom(playList.size() - 1)); } else {
	...
	public void checkMusic(int requestMusicId) {
		if (playListOn || settedMusic && playingMusicDelay + (180000) >= Utils.currentTimeMillis()) { return; }
		settedMusic = false;
	...
	public void sendHint(int musicIndex) {
		...
		if (musicId != -1) {
			player.getPackets().sendGameMessage("This track " +
					                                    (unlockedMusics.contains(musicId) ? "was unlocked" : "unlocks") + " " + MusicHints.getHint(musicId));
		}
	}
