...
public class HintIconsManager {
	...
	public HintIcon getIconAtTile(WorldTile tile) {
		for (HintIcon icon : loadedIcons) {
			if (icon == null) {
				continue;
			}
			if (icon.getWorldTile().matches(tile)) {
				return icon;
			}
		}
		return null;
	...
	public void removeIconAtTile(WorldTile tile) {
		HintIcon icon = getIconAtTile(tile);
		if (icon == null) {
			return;
		}
		int index = -1;
		for (int i = 0; i < loadedIcons.length; i++) {
			if (loadedIcons[i].equals(icon)) {
				index = i;
				break;
			}
		}
		removeHintIcon(index);
	}
