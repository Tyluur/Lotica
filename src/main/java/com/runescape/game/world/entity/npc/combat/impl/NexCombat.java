...
import com.runescape.game.interaction.cutscenes.NexCutScene;
		...
		if (nex.isFollowTarget()) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (count == 0) {
							...
							nex.calculatePossibleTargets(center, dir, idx == 0 || idx == 2).stream().filter(entity -> entity instanceof Player).forEach(entity -> {
								Player player = (Player) entity;
								player.getCutscenesManager().play(new NexCutScene(dir, idx));
								player.applyHit(new Hit(npc, Utils.getRandom(nex.getStage() == 4 ? 800 : 650), HitLook.REGULAR_DAMAGE));
								player.setNextAnimation(new Animation(10070));
								player.setNextForceMovement(new ForceMovement(player, 1, idx == 3 ? 3 : idx == 2 ? 2 : idx == 1 ? 1 : 0));
							});
						} else if (count == 3) {
			...
			if (Utils.getRandom(15) == 0) {
				...
				if (settedTarget != null && distance > 10) {
					...
					player.setNextForceMovement(new ForceMovement(nex, 2, Utils.getMoveDirection(npc.getCoordFaceX(player.getSize()) - player.getX(), npc.getCoordFaceY(player.getSize()) - player.getY())));
			...
			if (nex.getAttacksStage() == 0) {
						...
						public void run() {
							...
							if (Utils.getRandom(2) == 0) {
								for (Entity entity : nex.getPossibleTargets()) {
									if (entity instanceof Player) {
										Player player = (Player) entity;
										int distance = Utils.getDistance(player.getX(), player.getY(), npc.getX(), npc.getY());
				...
				for (final Entity t : npc.getPossibleTargets()) {
					...
					if (distance <= 10) {
						...
						delayHit(npc, 1, t, getRangeHit(npc, getRandomMaxHit(npc, damage, NPCCombatDefinitions.RANGE, t)));
						WorldTasksManager.schedule(new WorldTask() {
							@Override
							public void run() {
					...
					if (maxMinions != 0) {
						...
						for (int dir = 0; dir < dirs[0].length; dir++) {
							...
							if (World.canMoveNPC(tile.getPlane(), tile.getX(), tile.getY(), bloodReaverSize)) { // if found
								// done
								nex.getBloodReavers()[respawnedBloodReaverCount++] = new NPC(13458, tile, -1, true, true);
