...
public final class DominionTower implements Serializable {
	...
	public void handleButtons(int interfaceId, int componentId) {
		if (interfaceId == 1164) {
			if (componentId == 26) { openClimberMode(); } else if (componentId == 28) {
				openEnduranceMode();
			} else if (componentId == 29) {
				openSpecialMode();
			} else if (componentId == 30) {
				openFreeStyleMode();
			} else if (componentId == 31) { openSpectate(); }
		} else if (interfaceId == 1163) {
			if (componentId == 89) { player.closeInterfaces(); }
		} else if (interfaceId == 1168) {
			if (componentId == 254) { player.closeInterfaces(); }
		} else if (interfaceId == 1170) {
			if (componentId == 85) { player.closeInterfaces(); }
		} else if (interfaceId == 1173) {
			if (componentId == 58) { player.closeInterfaces(); } else if (componentId == 59) { startEnduranceMode(); }
		}
	}
	...
	private static final Boss[] BOSSES = { new Boss("Elvarg", "Grrrr", 14548), new Boss("Delrith", "Grrrr", -1, false, new Item(2402, 1), NORMAL_ARENA, 14578), new Boss("Evil Chicken", "Bwak bwak bwak", 3375), new Boss("The Black Knight Titan", "Kill kill kill!", 14436), new Boss("Bouncer", "Grrr", 14483),
			//custom bosses
			new Boss("Corporeal Beast", null, -1, true, null, NORMAL_ARENA, 8133), new Boss("Jad", "Roarrrrrrrrrrrrrrrrrrrrrrrrrr", 2745), new Boss("KalphiteQueen", null, 1158), new Boss("King Black Dragon", "Grrrr", 50), new Boss("Nomad", "You don't stand a chance!", 7985, true, null, NOMAD_ARENA, 8528), };
	...
	public int getNextBossIndex() {
		if (nextBossIndex < 0 || nextBossIndex >= BOSSES.length) { selectBoss(); }
	...
	public void startFight(final NPC[] bosses) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count == 0) {
					player.getInterfaceManager().sendTab(player.getInterfaceManager().onResizable() ? 11 : 0, 1172);
					...
					if (BOSSES[index].text != null) { bosses[0].setNextForceTalk(new ForceTalk(BOSSES[index].text)); }
					if (BOSSES[index].voice != -1) { player.getPackets().sendVoice(BOSSES[index].voice); }
				} else if (count == 6) {
	...
	public void win(int mode) {
		...
		progress++;
		if (mode == CLIMBER) {
			if (progress > maxFloorClimber) { maxFloorClimber = progress; }
		} else if (mode == ENDURANCE) {
			if (progress > maxFloorEndurance) { maxFloorEndurance = progress; }
		}
	...
	public void destroyArena(final boolean logout, int mode) {
		...
		if (logout) { player.setLocation(tile); } else {
			player.getControllerManager().removeController();
			player.getLockManagement().lockAll();
			player.setNextWorldTile(tile);
			if (mode == ENDURANCE) { progress = 0; }
		}
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
	...
	public void selectBoss() {
		if (nextBossIndex < 0 || nextBossIndex >= BOSSES.length) { nextBossIndex = Utils.random(BOSSES.length); }
	}
	...
	public void openEnduranceMode() {
		...
		player.getPackets().sendIComponentText(1173, 31, dominionFactor == MAX_FACTOR ? "" : String.valueOf(getBossesTotalLevel() * 10)); // on win
		player.getPackets().sendIComponentText(1173, 33, String.valueOf(dominionFactor)); // on death
	}

	public int getBossesTotalLevel() {
		int level = 0;
		for (int id : BOSSES[getNextBossIndex()].ids) { level = +NPCDefinitions.getNPCDefinitions(id).combatLevel; }
		return level;
	}
	...
	public void talkToFace(boolean fromDialogue) {
		...
		if (!talkedWithFace) { player.getDialogueManager().startDialogue("StrangeFace"); } else {
			if (!fromDialogue) { player.getPackets().sendVoice(7893); }
