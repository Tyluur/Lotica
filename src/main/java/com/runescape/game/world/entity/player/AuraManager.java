...
public class AuraManager implements Serializable {
	...
	public void process() {
		...
		if (Utils.currentTimeMillis() < activation) { return; }
		desactive();
	...
	public void activate() {
		...
		if (item == null) { return; }
		if (item.getDefinitions().containsOption("Discard")) { return; }
	...
	public int getAuraModelId2() {
		...
		switch (aura) {
			...
			case 23854: // Supreme harmony.
				return 68613;
			default:
				System.out.println("Unknown wings: " + aura);
				return -1;
		}
	}
	...
	public void checkSuccefulHits(int damage) {
		...
		int aura = player.getEquipment().getAuraId();
		if (aura == 22296) { useInspiration(); } else if (aura == 22298) { useVampyrism(damage); }
	}
