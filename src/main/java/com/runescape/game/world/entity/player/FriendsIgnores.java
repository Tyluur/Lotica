...
public class FriendsIgnores implements Serializable {
	...
	public HashMap<String, Integer> getFriendsChatRanks() {
		if (friendsChatRanks == null) {// temporary
			...
			for (String friend : friends) { friendsChatRanks.put(friend, 0); }
		}
	...
	public boolean isOnline(Player p2) {
		if (p2.getFriendsIgnores().privateStatus == 2) { return false; }
	...
	public void sendMessage(Player p2, String message) {
		...
		CoresManager.LOG_PROCESSOR.appendLog(new GameLog("private_message", player.getUsername(), "Sent " + p2.getUsername() + " private message saying: " + message));
	...
	public void changeRank(String username, int rank) {
		...
		if (p2 != null) { displayName = p2.getDisplayName(); } else {
	...
	public void handleFriendChatButtons(int interfaceId, int componentId, int packetId) {
		if (interfaceId == 1109) {
			if (componentId == 27) {
				...
				player.getFacade().setLootshareEnabled(!player.getFacade().isLootshareEnabled());
				player.refreshLootShare();
				if (player.getFacade().isLootshareEnabled()) {
					player.sendMessage("You now have lootshare enabled.");
				}
			}
	...
	public void refreshWhoCanEnterChat() {
		String text;
		if (whoCanEnterChat == 0) { text = "Any friends"; } else if (whoCanEnterChat == 1) {
			text = "Recruit+";
	...
	public void refreshWhoCanTalkOnChat() {
		String text;
		if (whoCanTalkOnChat == 0) { text = "Any friends"; } else if (whoCanTalkOnChat == 1) {
			text = "Recruit+";
	...
	public void refreshWhoCanKickOnChat() {
		String text;
		if (whoCanKickOnChat == 0) { text = "Any friends"; } else if (whoCanKickOnChat == 1) {
			text = "Recruit+";
	...
	public void refreshWhoCanShareloot() {
		String text;
		if (whoCanShareloot == 0) { text = "Any friends"; } else if (whoCanShareloot == 1) {
			text = "Recruit+";
	...
	public void removeIgnore(String username) {
		...
		if (!ignores.remove(formatedUsername) && !tillLogoutIgnores.remove(formatedUsername)) {
			if (p2 == null) { return; }
			if (!ignores.remove(p2.getUsername())) { tillLogoutIgnores.remove(p2.getUsername()); }
		}
	}
