...
public final class CombatDefinitions implements Serializable {
	...
	private transient boolean dungeoneeringSpellbook;
	...
	public int getSpellId() {
		Integer tempCastSpell = (Integer) player.getAttributes().get("tempCastSpell");
	...
	public static boolean isMiasmicSpell(int spellId) {
		return spellId >= 36 && spellId <= 39;
	}
	...
	public void refreshSpellBookScrollBar_DefCast() {
		player.getPackets().sendConfig(439, (dungeoneeringSpellbook ? 3 : spellBook) + (defensiveCasting ? 0 : 1 << 8));
	}
	...
	public void refreshSpellBook() {
		if (spellBook == 0) {
			player.getPackets().sendConfig(1376, sortSpellBook | (showCombatSpells ? 0 : 1 << 9) | (showSkillSpells ? 0 : 1 << 10) | (showMiscellaneousSpells ? 0 : 1 << 11) | (showTeleportSpells ? 0 : 1 << 12));
		} else if (spellBook == 1) {
			player.getPackets().sendConfig(1376, sortSpellBook << 3 | (showCombatSpells ? 0 : 1 << 16) | (showTeleportSpells ? 0 : 1 << 17));
		} else if (spellBook == 2) {
			player.getPackets().sendConfig(1376, sortSpellBook << 6 | (showCombatSpells ? 0 : 1 << 13) | (showMiscellaneousSpells ? 0 : 1 << 14) | (showTeleportSpells ? 0 : 1 << 15));
		}
	}
	...
	/**
	 * Gets the formatted bonus information including the leading -/+
	 *
	 * @param index
	 * 		The index for the bonus
	 */
	public String getBonus(int index) {
		if (index < 0 || index >= bonuses.length) {
			throw new ArrayIndexOutOfBoundsException();
		} else {
			int bonus = getBonuses()[index];
			if (bonus < 0) {
	...
	public void refreshBonuses() {
		...
		for (Item item : player.getEquipment().getItems().getItems()) {
			...
			int[] bonuses = ItemInformationLoader.getBonuses(item.getId());
			if (bonuses == null) {
				continue;
			}
	...
	public void restoreSpecialAttack() {
		...
		if (specialAttackPercentage == 100 || specialAttackPercentage == 50) {
			player.getPackets().sendGameMessage("Your special attack energy is now " + specialAttackPercentage + "%.", true);
		}
	}
	...
	public void setAttackStyle(int style) {
		...
		String name = weaponId == -1 ? "" : ItemDefinitions.forId(weaponId).getName().toLowerCase();
	...
	public boolean isDungeoneeringSpellbook() {
		return dungeoneeringSpellbook;
	}

	public void removeDungeoneeringBook() {
		if (dungeoneeringSpellbook) {
			dungeoneeringSpellbook = false;
			player.getInterfaceManager().sendMagicBook();
		}
	}
