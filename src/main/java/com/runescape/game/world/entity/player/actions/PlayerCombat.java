...
import com.runescape.game.world.entity.npc.fightkiln.HarAken;
import com.runescape.game.world.entity.npc.fightkiln.HarAkenTentacle;
import com.runescape.game.world.entity.npc.godwars.zaros.NexMinion;
import com.runescape.game.world.entity.npc.pest.PestPortal;
import com.runescape.game.world.entity.npc.qbd.QueenBlackDragon;
...
import com.runescape.game.world.entity.player.achievements.easy.Novice_Magician;
import com.runescape.game.world.entity.player.achievements.hard.Obsidian_Ringer;
...
public class PlayerCombat extends Action {
	...
	private boolean blitzSpell; // temporary constant
	 ...
	 * 		The player
	 */
	public static void checkSpecialToggle(Player player) {
		// if we're about to switch weapons, we need to switch the weapon then switch special attack
		// instead of switching special attack then weapon [big pking bug]
		if (player.getAttribute("switching_happening", false)) {
			return;
		}
	...
	public boolean process(Player player) {
		player.getDegradeManager().process(DegradeType.COMBATTED_DEGRADE);
		checkSpecialToggle(player);
	...
	/*	if (distanceX > size + maxDistance || distanceX < -1 - maxDistance || distanceY > size + maxDistance || distanceY < -1 - maxDistance) {
		...
		if (spellId > 0) {
			...
			spellcasterGloves = gloves != null && gloves.getDefinitions().getName().contains("Spellcaster glove") && player.getEquipment().getWeaponId() == -1 && Utils.percentageChance(45) ? spellId : -1;
	...
	private boolean checkAll(Player player) {
		if (!target.canBeAttacked(player) || player.isDead() || player.hasFinished() || target.isDead() || target.hasFinished()) {
			return false;
		}
		if (target.isNPC()) {
			NPC npc = target.npc();
			...
			if (npc.getId() == 1 && npc.getRespawnTile().matches(3081, 3486, 0)) {
				player.sendMessage("Leave thomas alone!");
				return false;
			}
			if (!SlayerMonsters.canAttack(player, npc, true)) {
				return false;
			}

			if (!npc.hasCombatDefinitions()) {
				if (GameConstants.DEBUG) {
					System.err.println(npc + " has no combat definitions; we should fix this.");
				}
			}
		}
		if (player.getAttribute("greegree_worn") != null) {
			player.sendMessage("You can't fight when you're in this form.");
			return false;
		}
		if ((target.isPlayer() && target.player().invalidDueToSecurityQuestions()) || player.getRandomEventManager().inRandomEvent()) {
			return false;
		}
		int distanceX = player.getX() - target.getX();
		int distanceY = player.getY() - target.getY();
		...
		if (player.getCombatDefinitions().getSpellBook() == 193 && CombatDefinitions.isMiasmicSpell(player.getCombatDefinitions().getSpellId()) && !player.getQuestManager().isFinished(DesertTreasure.class)) {
			...
			player.sendMessage("You must complete 'Desert Treasure' to use this spell.");
			return false;
		}
		if (target instanceof Player) {
			Player p2 = (Player) target;
			if (!player.isCanPvp() || !p2.isCanPvp() || p2.getRandomEventManager().inRandomEvent()) {
				return false;
			}
		} else {
			NPC n = (NPC) target;
		...
		int isRanging = isRanging(player);
		if (player.isFrozen()) {
			return !Utils.colides(player, target) && !(isRanging == 0 && target.getSize() == 1 && player.getCombatDefinitions().getSpellId() <= 0 && !hasPolyporeStaff(player) && Math.abs(player.getX() - target.getX()) == 1 && Math.abs(player.getY() - target.getY()) == 1 && !target.hasWalkSteps());
		}
		//colission check
		if (Utils.colides(player, target)) {
			if (target.hasWalkSteps()) { target.resetWalkSteps(); }
			if (player.hasWalkSteps()) { player.resetWalkSteps(); }
			return player.calcFollow(target, true);
		}
		// diagonal check
		if (isRanging == 0 && target.getSize() == 1 && player.getCombatDefinitions().getSpellId() <= 0 && !hasPolyporeStaff(player) && Math.abs(player.getX() - target.getX()) == 1 && Math.abs(player.getY() - target.getY()) == 1 && !target.hasWalkSteps()) {
		...
		//range check
		if (!Utils.isOnRange(player, target, maxDistance) || !player.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target))) {
		...
		if (checkInstantSpecials(player)) { return true; }
		return true;
	}
	...
	private boolean checkInstantSpecials(Player player) {
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double specMultiplier, boolean usingSpec) {
		...
		if (defenceAffects) {
			...
			if (target.isNPC() && SlayerManagement.isTask(player, target.npc())) {
				def *= SlayerManagement.equippingBlackMask(player) ? 0.84 : 1;
			}
			if (usingSpec) {
				double multiplier = getSpecialAccuracyModifier(player);
				att *= multiplier;
			}
			if (!Combat.rollHit(att, def)) { return 0; }
		}
		if (target.isNPC() && SlayerManagement.isTask(player, target.npc())) {
			max_hit *= SlayerManagement.equippingBlackMask(player) ? 1.15 : 1;
		}
	...
	private void delayHit(int delay, final int weaponId, final int attackStyle, final Hit... hits) {
		...
		@SuppressWarnings("unused") final boolean reduceAttack = this.reduceAttack;
			...
			if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
				...
				if (combatXp > 0) {
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE) {
						if (attackStyle == 2) {
							if (target.isPlayer()) {
								player.getSkills().addXpNoModifier(Skills.RANGE, combatXp / 2);
								player.getSkills().addXpNoModifier(Skills.DEFENCE, combatXp / 2);
							} else {
						...
						if (xpStyle != CombatDefinitions.SHARED) {
							if (target.isPlayer()) {
								player.getSkills().addXpNoModifier(xpStyle, combatXp);
							} else { player.getSkills().addXp(xpStyle, combatXp); }
						} else {
							if (target.isPlayer()) {
								player.getSkills().addXpNoModifier(Skills.ATTACK, combatXp / 3);
								player.getSkills().addXpNoModifier(Skills.STRENGTH, combatXp / 3);
								player.getSkills().addXpNoModifier(Skills.DEFENCE, combatXp / 3);
							} else {
					...
					if (hpXp > 0) {
						if (target.isPlayer()) {
							player.getSkills().addXpNoModifier(Skills.HITPOINTS, hpXp);
						} else {
				...
				if (mage_hit_gfx != 0 && damage > 0) {
					if (freeze_time > 0) {
						target.addFreezeDelay(freeze_time, false, player);
						if (freeze_time > 0) {
							if (target instanceof Player) {
				...
				if (combatXp > 0) {
					...
					if (player.getCombatDefinitions().isDefensiveCasting() || (hasPolyporeStaff(player) && player.getCombatDefinitions().getAttackStyle() == 1)) {
						...
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							if (target.isPlayer()) {
								player.getSkills().addXpNoModifier(Skills.DEFENCE, defenceXp / 7.5);
							} else { player.getSkills().addXp(Skills.DEFENCE, defenceXp / 7.5); }
						}
					}
					if (target.isPlayer()) { player.getSkills().addXpNoModifier(Skills.MAGIC, combatXp); } else {
						player.getSkills().addXp(Skills.MAGIC, combatXp);
					}
					double hpXp = damage / 7.5;
					if (hpXp > 0) {
						if (target.isPlayer()) { player.getSkills().addXpNoModifier(Skills.HITPOINTS, hpXp); } else {
							player.getSkills().addXp(Skills.HITPOINTS, hpXp);
						}
					}
				}
				...
				for (Hit hit : hits) {
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE || hit.getLook() == HitLook.MELEE_DAMAGE) {
						...
						if (combatXp > 0) {
							if (hit.getLook() == HitLook.RANGE_DAMAGE) {
								if (weaponId != -1) {
									String name = ItemDefinitions.forId(weaponId).getName();
									if (name.contains("(p++)")) {
							...
							if (mage_hit_gfx != 0) {
								...
								if (block_tele) {
									if (target instanceof Player) {
										...
										targetPlayer.getPackets().sendGameMessage("A teleport block, which is active for " + targetPlayer.getTeleBlockTimeleft() + ", has been cast on you.", true);
									}
								}
							}
							if (magic_sound > 0) {
		...
		if (delay <= 1) { doDefenceEmote(); } else {
			int thirdTaskDelay = delay - 1;
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					doDefenceEmote();
				}
			}, thirdTaskDelay < 0 ? 0 : thirdTaskDelay);
		}
	}
	...
	public static final boolean fullVeracsEquipped(Player player) {
		...
		return ItemDefinitions.forId(helmId).getName().contains("Verac's") && ItemDefinitions.forId(chestId).getName().contains("Verac's") && ItemDefinitions.forId(legsId).getName().contains("Verac's") && ItemDefinitions.forId(weaponId).getName().contains("Verac's");
	}
	...
	public static boolean fullDharokEquipped(Player player) {
		...
		int legsId = player.getEquipment().getLegsId();
		int weaponId = player.getEquipment().getWeaponId();
		return !(helmId == -1 || chestId == -1 || legsId == -1 || weaponId == -1) && ItemDefinitions.forId(helmId).getName().contains("Dharok's") && ItemDefinitions.forId(chestId).getName().contains("Dharok's") && ItemDefinitions.forId(legsId).getName().contains("Dharok's") && ItemDefinitions.forId(weaponId).getName().contains("Dharok's");
	}
	...
	public static final boolean fullBattlemageEquipped(Player player) {
		...
		int chestId = player.getEquipment().getChestId();
		int legsId = player.getEquipment().getLegsId();
		int glovesId = player.getEquipment().getWeaponId();
		int bootsId = player.getEquipment().getWeaponId();
		if (helmId == -1 || chestId == -1 || legsId == -1 || glovesId == -1 || bootsId == -1) {
			return false;
		}
		return ItemDefinitions.forId(helmId).getName().contains("Battle-mage") && ItemDefinitions.forId(chestId).getName().contains("Battle-mage") && ItemDefinitions.forId(legsId).getName().contains("Battle-mage") && ItemDefinitions.forId(glovesId).getName().contains("Battle-mage") && ItemDefinitions.forId(bootsId).getName().contains("Battle-mage");
	}
	...
	public static final boolean fullVanguardEquipped(Player player) {
		...
		return ItemDefinitions.forId(helmId).getName().contains("Vanguard") && ItemDefinitions.forId(chestId).getName().contains("Vanguard") && ItemDefinitions.forId(legsId).getName().contains("Vanguard") && ItemDefinitions.forId(weaponId).getName().contains("Vanguard") && ItemDefinitions.forId(bootsId).getName().contains("Vanguard") && ItemDefinitions.forId(glovesId).getName().contains("Vanguard");
	}
	 ...
	 * target; if (n.getApplicableIds() == 9463 && hasFireCape(player)) hit += 40; } if
	...
	public static boolean canSlashWeb(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
		String name = ItemDefinitions.forId(weaponId).getName().toLowerCase();
		return player.getInventory().contains(946) || name.contains("sword") || name.contains("claw") || name.contains("scimitar") || name.contains("dagger") || name.contains("halberd") || name.contains("");
	}
	...
	public boolean isWithinDistance(Player player, Entity target) {
		...
		if (!player.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target)) || !Utils.isOnRange(player, target, maxDistance
	...
	private int getRangeCombatDelay(int weaponId, int attackStyle) {
		...
		if (weaponId != -1) {
			...
			if (weaponName.contains("shortbow") || weaponName.contains("karil's crossbow") || weaponName.contains("sling")) {
				...
				delay = 4;
			} else if (weaponName.contains("crystal bow") || weaponName.contains("toktz-xil-ul")) {
				delay = 3;
			} else {
				switch (weaponId) {
					case 15241:
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			switch (spellId) {
				case 25: // air strike
					...
					AchievementHandler.incrementProgress(player, Novice_Magician.class, 1);
					return 5;
				...
				case 36:// bind
					...
					if (magicHit.getDamage() > 0 && target.getFrozenBlockedDelay() < currentTime) {
						target.addFreezeDelay(5000, true, player);
					}
					return 5;
				case 55:// snare
					player.setNextGraphics(new Graphics(177));
					...
					if (snareHit.getDamage() > 0 && target.getFrozenBlockedDelay() < Utils.currentTimeMillis()) {
						target.addFreezeDelay(10000, true, player);
					}
					World.sendProjectile(player, target, 178, 18, 18, 50, 50, 0, 0);
					return 5;
				case 81:// entangle
					...
					if (entangleHit.getDamage() > 0 && target.getFrozenBlockedDelay() < Utils.currentTimeMillis()) {
						target.addFreezeDelay(20000, true, player);
					}
					World.sendProjectile(player, target, 178, 18, 18, 50, 50, 0, 0);
					return 5;
				...
				case 86: // teleblock
					if (target instanceof Player && ((Player) target).getTeleBlockDelay() <= Utils.currentTimeMillis()) {
						...
						if (target instanceof Player) {
							Player targetPlayer = (Player) target;
							teleBlockTime = (targetPlayer.getPrayer().usingPrayer(0, 17) || targetPlayer.getPrayer().usingPrayer(1, 7) ? 150000 : 300000);
						}
				...
				case 99: // Storm of armadyl //Sonic and Tyler dumped
					...
					int hit = getRandomMagicMaxHit(player, (target.isPlayer() ? 240 : 160) + boost);
					if (hit > 0 && hit < boost) {
						hit += boost;
					}
					...
					if (target.isPlayer()) {
						target.player().getSkills().setLevel(Skills.DEFENCE, target.player().getSkills().getLevel(Skills.DEFENCE) - 1);
					}
			...
			switch (spellId) {
				...
				case 21:// Ice Blitz
					...
					magic_sound = 169;
					blitzSpell = true;
					delayMagicHit((Utils.getDistance(player, target) > 3 ? 4 : 2), getMagicHit(player, getRandomMagicMaxHit(player, 260)));
				...
				case 23: // ice barrage
					...
					attackTarget(getMultiAttackTargets(player), new MultiAttack() {
						...
						public boolean attack() {
							...
							int hitDelay = Utils.getDistance(player, target) > 3 ? 4 : 2;

							delayMagicHit(hitDelay/* - (hitDelay == 2 ? 0 : 1)*/, hit);
							World.sendProjectile(player, target, 368, 60, 32, 50, 50, 0, 0);
							if (!nextTarget) {
								if (damage == -1) {
									return false;
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			switch (weaponId) {
				...
				case 14684: // zanik cbow
					...
					delayHit(2, weaponId, attackStyle, getRangeHit(player, (int) (damage * 1.25)));
					dropAmmo(player, 1);
					break;
				case 13954:// morrigan javelin
				case 12955:
			...
			if (weaponId != -1) {
				String weaponName = ItemDefinitions.forId(weaponId).getName().toLowerCase();
				if (weaponName.contains("toktz-xil-ul") || weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName.contains("thrownaxe")) {
					...
					dropAmmo(player, -1);
					if (weaponId == 6522) {
						AchievementHandler.incrementProgress(player, Obsidian_Ringer.class, 1);
					}
				} else if (weaponName.contains("crossbow")) {
					int damage;
					int ammoId = player.getEquipment().getAmmoId();
					...
					World.sendProjectile(player, target, 249, 50, 36, 41, 35, 0, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					...
					if (getRandomMaxHit(player, weaponId, attackStyle, true) > 0 && target.getFrozenBlockedDelay() < currentTime) {
						target.addFreezeDelay(delay, true, player);
						WorldTasksManager.schedule(new WorldTask() {
							@Override
							public void run() {
					...
					player.setNextGraphics(new Graphics(getArrowThrowGfxId(weaponId, ammoId), 0, 100));
					World.sendProjectile(player, target, getArrowProjectileGfxId(weaponId, ammoId), 41, 36, 10 + (Utils.getDistance(player, target) * 5), 35, 16, 0);
					int hit = getRandomMaxHit(player, weaponId, attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
	...
	private void checkSwiftGlovesEffect(Player player, int hitDelay, int attackStyle, int weaponId, int hit, int gfxId, int startHeight, int endHeight, int speed, int delay, int curve, int startDistanceOffset) {
		...
		if (hit > (max_hit - 10)) {
			target.addFreezeDelay(10000, false, player);
		...
		if (quantity == -2) {
			...
			if (weaponId != -1) {
				if ((quantity == -3 && Utils.getRandom(10) < 2) || (quantity != -3 && Utils.getRandom(3) > 0)) {
					int capeId = player.getEquipment().getCapeId();
					if (capeId != -1 && ItemDefinitions.forId(capeId).getName().contains("Ava's") || capeId == 20771) {
	...
	private int meleeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			switch (weaponId) {
				...
				case 11716:
					if (target instanceof Player) {
						...
						other.getLockManagement().lockActions(3000, LockType.ACTION, LockType.FOOD, LockType.INTERFACE_INTERACTION, LockType.WALKING, LockType.ITEM_INTERACTION);
					} else {
						NPC n = (NPC) target;
					...
					boolean left = player.getX() > target.getX();
					boolean right = player.getX() < target.getX();
					boolean down = player.getY() > target.getY();
					boolean up = player.getY() < target.getY();
					...
					target.addWalkSteps(newX, newY, 1);
				...
				case 11700:
					...
					if (zgsdamage != 0 && target.getSize() <= 1) { // freezes small
						...
						target.addFreezeDelay(18000, false, player); // 18seconds
					}
				...
				case 4587: // dragon sci
					...
					if (target instanceof Player) {
						Player p2 = (Player) target;
						if (hit1.getDamage() > 0) {
							p2.putAttribute("PrayerBlocked", 5000 + Utils.currentTimeMillis());
							if (p2.getPrayer().isAncientCurses()) {
								int[] prayerIds = new int[] { 6, 7, 8, 9, 17, 18 };
								p2.getPrayer().closePrayers(prayerIds);
							} else {
								int[] prayerIds = new int[] { 16, 17, 18, 19 };
								p2.getPrayer().closePrayers(prayerIds);
							}
				...
				case 1434: // dragon mace
					...
					break;
				case 7158:
					player.setNextAnimation(new Animation(7048));
			...
			if (fullArmourSetEquipped(player, "Guthan's") && (Utils.percentageChance(25) || GameConstants.DEBUG)) {
