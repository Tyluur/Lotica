...
public final class LocalPlayerUpdate {
	...
	public boolean needAppearenceUpdate(int index, byte[] hash) {
		return !(totalRenderDataSentLength > ((GameConstants.PACKET_SIZE_LIMIT - 500) / 2) || hash == null) && (cachedAppearencesHashes[index] == null || !MessageDigest.isEqual(cachedAppearencesHashes[index], hash));
	}
			...
			if (needsRemove(p)) {
				...
				if (p.hasTeleported()) {
					...
					if ((dx == 0 && dy == 0)) {
						...
						if (!needUpdate) {
							appendUpdateBlock(p, updateBlockData, false, false);
						}
					} else {
						stream.writeBits(1, needUpdate ? 1 : 0);
